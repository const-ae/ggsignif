test_that("should not work", {
  ...
  expect_error(print(ggplot(mpg, aes(x = manufacturer, y = displ)) +
    geom_boxplot() +
    geom_signif(
      comparisons = list(c("audi", "ford"), c("hyundai", "nissan")),
      map_signif_level = TRUE,
  ...
  expect_error(print(ggplot(airquality_trimmed, aes(x = Month, y = Ozone, fill = Temp.f)) +
  ...
  expect_error(print(ggplot(iris, aes(x = Species, y = Sepal.Length)) +
    geom_boxplot() +
    geom_signif(
